...
### [2026-05-20] Docker extraction fails with "Failed to connect to extraction stream" — pino crashes in Turbopack production build

- **Error:** `docker compose up` starts the app, health endpoint returns OK, but uploading any PDF causes "Extraction failed — Failed to connect to extraction stream." Server logs show: `Error: default level: must be included in custom levels` from a Turbopack-generated chunk.
- **Context:** Docker production build using `npm run build` (Turbopack, the Next.js 16 default). `serverExternalPackages` includes `pino`. Dev mode (`npm run dev`) works fine.
- **Root Cause:** Turbopack's externals resolver renames packages with hash suffixes (e.g., `require("pino-28069d5257187539")`) in the bundled server chunks. At runtime, Turbopack's module resolution maps this hash-suffixed name back to the real package, but pino v10's internal worker-thread initialization fails because its thread-stream sub-modules can't resolve correctly in this context. The webpack build (`next build --webpack`) correctly externalizes pino but fails on ESM-only packages like `archiver` and rejects `__resetForTests` exports on route files.
- **Fix:** Wrapped pino initialization in `src/lib/logger.ts` with a try/catch that falls back to a thin console-based logger when pino fails to initialize. Pino remains the primary logger — the fallback only activates in the Turbopack production bundle edge case. Also added `postinstall` workaround to Docker build for the pdfjs worker file (skipped by `--ignore-scripts`).
- **Prevention:** If Next.js / Turbopack fixes their externals resolver in a future version, the try/catch becomes a no-op and pino loads normally. The fallback is intentionally minimal (info/error/warn/debug/fatal → console) to avoid masking future pino-specific features.
